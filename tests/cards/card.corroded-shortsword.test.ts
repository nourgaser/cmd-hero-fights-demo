...
describe('card.corroded-shortsword', () => {
  it('summons Corroded Shortsword and it can attack once', () => {
    ...
    const summon = TEST_QUERIES.findEntityByCard(sim, CARD_IDS.corrodedShortsword)
    const summonPreview = TEST_QUERIES.getEntityPreview(sim, summon.entityId)
    expect(summon.kind).toBe('weapon')
    expect(summonPreview.currentHealth).toBe(14)
