import type { SummonedEntityBlueprint } from "../../../engine/actions/effects/context";
import type { EntityFootprint } from "../../../shared/models";
import { SUMMON_ENTITY_IDS } from "../constants";
import { KEYWORD_IDS } from "../../keywords";
import type { CommanderXSummonedDefinition } from "./types";

export const STEELBOUND_EFFIGY_BLUEPRINT: SummonedEntityBlueprint = {
  kind: "totem",
  definitionCardId: "card.commander-x.steelbound-effigy",
  keywordIds: [KEYWORD_IDS.taunt],
  maxHealth: 15,
  armor: 0,
  magicResist: 1,
  attackDamage: 0,
  abilityPower: 0,
  criticalChance: 0,
  criticalMultiplier: 1,
  dodgeChance: 0,
  maxMovesPerTurn: 0,
  remainingMoves: 0,
};

export const STEELBOUND_EFFIGY_FOOTPRINT: EntityFootprint = [
  { row: 0, column: 0 },
];

export default {
  entityId: SUMMON_ENTITY_IDS.steelboundEffigy,
  blueprint: STEELBOUND_EFFIGY_BLUEPRINT,
  footprint: STEELBOUND_EFFIGY_FOOTPRINT,
} satisfies CommanderXSummonedDefinition;
